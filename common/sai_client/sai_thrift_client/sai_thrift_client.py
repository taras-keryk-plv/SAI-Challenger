...
class SaiThriftClient(SaiClient):
    ...
    def generate_metadata(self):
        ...
        # Search for thrift adapter and meta data modules
        sai_thrift_adapter = glob.glob("/usr/local/lib/**/sai_adapter.py", recursive=True)[0]
        sai_thrift_meta = glob.glob("/usr/local/lib/**/sai_thrift_metadata.py", recursive=True)[0]
        sai_thrift_meta_new = sai_thrift_meta + "-new"
        sai_thrift_meta_backup = sai_thrift_meta + "-old"
        ...
        # Generate new meta data from sai adapter and compare with current
        with open(sai_thrift_meta_new, 'w') as f:
            f.write(f"# AUTOGENERATED using:\n# {cmd}\n")
            f.write("SaiMetadata = {\n")
        ...
        # Update if needed
        sum1 = subprocess.run(['md5sum', sai_thrift_meta_new], capture_output=True, text=True).stdout.split()[0]
        sum2 = subprocess.run(['md5sum', sai_thrift_meta], capture_output=True, text=True).stdout.split()[0]
        if sum1 == sum2:
            return
        subprocess.run(['cp', sai_thrift_meta, sai_thrift_meta_backup])
        subprocess.run(['cp', sai_thrift_meta_new, sai_thrift_meta])
